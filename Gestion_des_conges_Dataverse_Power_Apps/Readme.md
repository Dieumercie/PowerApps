...
📸 Captures recommandées pour ton GitHub

Modèle de données (tables + relations).

Formulaire Employé avec onglet Historique.

Vue Demandes de congés (En attente / Approuvé / Refusé).

Canvas App (formulaire de demande + galerie d’historique).

Exemple de flux Power Automate (Validation solde).
